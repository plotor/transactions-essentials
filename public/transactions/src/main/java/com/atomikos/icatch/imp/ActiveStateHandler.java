        ...
        if (orphansExist()) {
            try {
                if (LOGGER.isTraceEnabled()) {
                    LOGGER.logTrace("Orphans detected: " + getCoordinator().getLocalSiblingCount() + " vs " + globalSiblingCount_ + " - forcing rollback.");
                }
                rollbackWithAfterCompletionNotification(() -> rollbackFromWithinCallback(false, false));
        ...
        try {
            ...
            while (enumm.hasMoreElements()) {
                ...
                if (getCascadeList() != null && p.getURI() != null) { //null for OTS
                    Integer sibnum = getCascadeList().get(p.getURI());
