...
public class XAResourceTransaction implements ResourceTransaction, Participant {
    ...
    public synchronized int prepare() throws RollbackException,
        ...
        try {
            ...
            // 调用 XAResource#prepare 方法
            ret = this.xaresource.prepare(this.xid);

        } catch (XAException xaerr) {
            String msg = interpretErrorCode(this.resourcename, "prepare", this.xid, xaerr.errorCode);
     ...
     *
     * @param xaresource The new XAResource to use. This new XAResource represents the
